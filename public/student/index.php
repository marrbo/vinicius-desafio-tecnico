...
if(strcasecmp($_SERVER['REQUEST_METHOD'], 'post') == 0){
    ...
    // verifica se turma existe, se não,  bad request
    $hasTeam = Teams::has($data['team_id']);
    if(!$hasTeam){
        $resp = array( 'error' => 'Insira uma turma válida');
        echo json_encode($resp);
        http_response_code(401);
        die;
    }
    ...
    if(isset($data['_method']) && strcasecmp($data['_method'], 'put') == 0){
        ...
        $teamData = Students::edit($_GET['id'], $data);
        if(is_int($teamData) && $teamData > 0){
